...
# ─────────────────────────────────────────────────────────────────────────────
# deploy.sh — Déploiement du microservice boxify
# Usage : ./deploy.sh
# ─────────────────────────────────────────────────────────────────────────────
...
docker compose exec boxify python manage.py migrate --noinput

info "Création du superutilisateur (ignoré s'il existe déjà)..."
docker compose exec boxify python manage.py createsuperuser --noinput 2>/dev/null \
...
echo ""
echo -e "  API      : ${BLUE}http://localhost:${HOST_PORT}/api/items/${NC}"
